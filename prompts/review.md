...
- `items` 只列未通過的驗收條件與額外發現的重要問題；全部通過時用空陣列。每筆的 `status` 為 `not_met` 或 `partial`，`note` 請寫出具體位置與修正方向，例如 `{ "criterion": "AC-1", "status": "not_met", "note": "src/form.tsx 缺少 API 失敗時的錯誤訊息" }`。
</output_format>
